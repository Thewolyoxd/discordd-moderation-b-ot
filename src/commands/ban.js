...
if (!jyros) { return message.channel.send(embed.setColor(`${config.embed.color.red}`).setDescription(`${settings.emojis.no} Hatalı Kullanım! \`${config.PREFIX}ban @Kişi <sebep>\` `)).then(x => x.delete({ timeout: 5000 })); }
