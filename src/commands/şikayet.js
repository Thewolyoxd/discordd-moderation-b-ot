...
if (!jyros){message.channel.send(`${settings.emojis.no} ${message.author}, Şikayetçi olduğun kişiyi etiketleyip şikayetini yazmalısın. \`${config.PREFIX}şikayet @Kişi/ID <chat-sesli-taciz-dm-diğer>\``).then(m => m.delete({ timeout: 7000 })); message.react(settings.emojis.no)}
...
if(tür !== "chat" && tür !== "sesli" && tür !== "taciz" && tür !== "dm" && tür !== "diğer") return message.channel.send(`Eksik argüman! \`${config.PREFIX}şikayet <@Kişk/ID> <chat-sesli-taciz-dm-diğer>\``).then(x => x.delete({timeout: 5000})); 
